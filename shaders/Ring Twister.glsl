...
vec4 slice(float x0, float x1, vec2 uv) {
  ...
  // Lighting
  col *= w / sqrt(2.0 * IN_RADIUS * IN_RADIUS *
                  (1.0 - cos(tau / float(NUM_FACES))));
...
#ifdef ANTI_ALIAS
  col *= (-1.0 +
          2.0 * smoothstep(-0.03, 0.03, sin(u * pi * 4.0) * cos(uv.y * 16.0))) *
             (1.0 / 16.0) +
         0.7;
#else
...
#ifdef ANTI_ALIAS
  clip = (1.0 - smoothstep(0.5 - aaSize / w, 0.5 + aaSize / w, abs(u - 0.5))) *
         step(x0, x1);
#else
...
void main() {
  aaSize = 2.0 / resolution.y; // Scale anti aliasing with resolution
